...
function meaningNetworkContinuityRows(
  ...
  return edges
    .filter((edge) =>
      /continuity|tracks_same_entity|co_occurs|conflict|appearance|traceable_similarity|constellational_match/i.test(edge.edge_type),
    )
    .map((edge) => {
...
type MeaningNetworkMatcherQueueState = {
...
type MeaningNetworkReadingLens = "sfl_ideational" | "sfl_interpersonal" | "sfl_textual" | "moral_virtue";
type MeaningNetworkReadingStatus = "candidate" | "confirmed" | "rejected" | "deferred";
...
type MeaningNetworkInterpretiveReadingRecord = {
  schema: "vaa1.meaning_network_interpretive_reading.v1";
  reading_id: string;
  target_kind: "node" | "edge";
  target_id: string;
  reading_lens: MeaningNetworkReadingLens;
  status: MeaningNetworkReadingStatus;
  sfl_layer: MeaningNetworkSflLayer;
  sfl_category: string;
  judgement_group: string;
  judgement_axis: string;
  ...
  source_verification_status: "source_time_resolved" | "source_anchor_missing";
  source_range_source?: "evidence_ref" | "presence_override" | "manual_interval";
  source_anchors: Array<Record<string, unknown>>;
  evidence_refs: MeaningNetworkEvidenceRef[];
  source_traceback_refs: string[];
  authority_level: "manual_confirmation" | "manual_rejection" | "analyst_candidate";
  maturity_state: string;
...
const MEANING_NETWORK_DEFAULT_LANES: MeaningNetworkLane[] = [
  ...
  { lane_id: "matcher", label: "Matcher / SOM candidates", nodeTypes: ["matcher_anchor", "matcher_candidate"] },
...
const MEANING_NETWORK_SFL_VICE_FAMILY_LABELS: Record<string, string> = {
  wisdom_and_knowledge: "Intellectual closure",
  courage: "Cowardice / deception",
  humanity: "Cruelty / alienation",
  justice: "Corruption / oppression",
  temperance: "Arrogance / compulsion",
  transcendence: "Nihilism / despair",
};

function meaningNetworkViceFamilyLabel(family: string): string {
  return MEANING_NETWORK_SFL_VICE_FAMILY_LABELS[family] || family.replaceAll("_", " ");
}

function meaningNetworkViceAxisLabel(family: string, axis: string): string {
  const values = MEANING_NETWORK_SFL_VIRTUES[family]?.[axis]?.antithesis || [];
  if (values.length > 0) {
    return values.slice(0, 2).map((value) => value.replaceAll("_", " ")).join(" / ");
  }
...
function meaningNetworkReadingLensForDraft(
  draft: MeaningNetworkSflDraft,
): MeaningNetworkReadingLens {
  if (draft.sfl_layer === "virtues_and_vices") {
    return "moral_virtue";
  }
  return `sfl_${draft.sfl_layer}` as MeaningNetworkReadingLens;
}

function meaningNetworkReadingLensLabel(lens: MeaningNetworkReadingLens): string {
  if (lens === "moral_virtue") {
    return "Moral / virtue";
  }
  return lens.replace("sfl_", "SFL: ").replaceAll("_", " ");
}

function meaningNetworkSflTaxonomyPath(draft: MeaningNetworkSflDraft): string {
  if (draft.sfl_layer === "virtues_and_vices") {
    return `virtues_and_vices.${draft.virtue_family}.${draft.virtue_axis}.${draft.moral_polarity === "vice" ? "antithesis" : "positive"}.${draft.virtue_vice_value}`;
  }
  if (draft.sfl_layer === "interpersonal") {
    return `sfl.interpersonal.judgement_categories.${draft.judgement_group}.${draft.judgement_axis}`;
  }
  return `sfl.${draft.sfl_layer}.${draft.sfl_category}`;
}

function meaningNetworkReadingId({
  kind,
  id,
  taxonomyPath,
}: {
  ...
  const target = String(id || "target").replace(/[^a-zA-Z0-9_.:-]+/g, "_");
  const taxonomy = String(taxonomyPath || "reading").replace(/[^a-zA-Z0-9_.:-]+/g, "_");
  return `${kind}:${target}:reading:${taxonomy}`;
}

function meaningNetworkReadingStatusForDecision(
  decision: MeaningNetworkReadingDecision | string,
): MeaningNetworkReadingStatus {
...
function buildMeaningNetworkInterpretiveReadingRecord({
  kind,
  id,
  draft,
  decision,
  ...
  decision: MeaningNetworkReadingDecision | string;
  refs: MeaningNetworkEvidenceRef[];
  range: MeaningNetworkTimeRange | null;
  tracebackRefs: string[];
  now: string;
}): MeaningNetworkInterpretiveReadingRecord {
  const taxonomyPath = meaningNetworkSflTaxonomyPath(draft);
  const plainLanguageMeaning = meaningNetworkSflPlainLanguage(draft);
  const status = meaningNetworkReadingStatusForDecision(decision);
  ...
  return {
    schema: "vaa1.meaning_network_interpretive_reading.v1",
    reading_id: meaningNetworkReadingId({ kind, id, taxonomyPath }),
    target_kind: kind,
    target_id: id,
    reading_lens: meaningNetworkReadingLensForDraft(draft),
    status,
    ...
    applied_label: draft.applied_label || plainLanguageMeaning,
    ...
    maturity_state:
      status === "confirmed"
        ? "analyst_confirmed_interpretive_reading"
        : status === "rejected"
          ? "analyst_rejected_interpretive_reading"
          : "analyst_candidate_interpretive_reading",
    user_correction_stands: true,
    propagation_required: status === "confirmed",
    partial_propagation_allowed: false,
    created_at: now,
    created_by: "analyst",
  };
...
function meaningNetworkReadingMaturityResult(decision: MeaningNetworkReadingDecision | string): string {
  if (decision === "confirmed") {
    return "analyst_confirmed_interpretive_reading";
  }
  if (decision === "canceled") {
    return "analyst_rejected_interpretive_reading";
  }
  if (decision === "deferred") {
    return "analyst_deferred_interpretive_reading";
  }
  return "analyst_candidate_interpretive_reading";
}

function meaningNetworkReadingSaveTitle(decision: MeaningNetworkReadingDecision | string): string {
  if (decision === "confirmed") {
    return "Interpretive reading confirmed";
  }
  if (decision === "canceled") {
    return "Interpretive reading rejected";
  }
  if (decision === "deferred") {
    return "Interpretive reading deferred";
  }
  return "Candidate reading added";
}

function isMeaningNetworkInterpretiveReadingRecord(
  value: unknown,
): value is MeaningNetworkInterpretiveReadingRecord {
  return Boolean(
    value &&
      typeof value === "object" &&
      (value as MeaningNetworkInterpretiveReadingRecord).schema ===
        "vaa1.meaning_network_interpretive_reading.v1" &&
      typeof (value as MeaningNetworkInterpretiveReadingRecord).reading_id === "string",
  );
}
...
function meaningNetworkNodePriority(node: MeaningNetworkNode): number {
  ...
  if (isMatcherSomMeaningNode(node)) return 1;
...
function isMatcherSomMeaningNode(node: MeaningNetworkNode): boolean {
  const canonicalType = meaningNetworkCanonicalNodeType(node.node_type);
  return (
    canonicalType === "matcher_anchor" ||
    canonicalType === "matcher_candidate" ||
    String(node.ui?.display_group || "") === "open_topology_som_matcher" ||
    Boolean(node.attributes?.open_topology_som)
  );
}

function isMatcherSomMeaningEdge(edge: MeaningNetworkEdge): boolean {
  return (
    /matcher|som|traceable_similarity|constellational_match/i.test(edge.edge_type) ||
    Boolean((edge as any).attributes?.open_topology_som)
  );
}

function meaningNetworkMatcherTargetForNode(node: MeaningNetworkNode): string {
  const type = meaningNetworkCanonicalNodeType(node.node_type);
  if (["narrative_agent", "character", "identity", "speaker"].includes(type)) {
    return "character_continuity";
  }
  if (["transcript", "spoken_word", "prosody"].includes(type)) {
    return "speaker_voice_continuity";
  }
  if (["location", "scene", "setting"].includes(type)) {
    return "scene_setting";
  }
  if (type === "object") return "visual_pattern";
  return "named_entity";
}

function meaningNetworkMatcherCandidateLabel(candidate: EvidenceProliferationCandidate): string {
  return String(
    candidate.label ||
      candidate.master_object_projection?.current_label ||
...
function meaningNetworkMatcherCandidateConfidence(candidate: EvidenceProliferationCandidate): number {
  return Number(
    candidate.match_probability ??
      candidate.match_score ??
      candidate.master_object_projection?.confidence ??
      0,
  );
}

type MeaningNetworkMatcherCandidateRole =
  | "anchor_sample"
  | "identity_candidate"
  | "context_support"
  | "conflict";

function meaningNetworkMatcherCandidateRole(
  candidate: EvidenceProliferationCandidate,
): MeaningNetworkMatcherCandidateRole {
  if (
    candidate.candidate_role === "anchor_sample" ||
    candidate.candidate_role === "identity_candidate" ||
    candidate.candidate_role === "context_support" ||
    candidate.candidate_role === "conflict"
  ) {
    return candidate.candidate_role;
  }
  const compatibility = candidate.closest_match?.identity_compatibility;
  if (compatibility === "contextual_support") return "context_support";
  if (compatibility === "incompatible") return "conflict";
  if (
    compatibility === "direct_identity" &&
...
function meaningNetworkMatcherCandidateBbox(
  ...
  const source = (
    (geometry.bbox && typeof geometry.bbox === "object" ? geometry.bbox : null) ||
    (geometry.roi && typeof geometry.roi === "object" ? geometry.roi : null) ||
    geometry
  ...
  const width = Number(
    source.width ??
      source.w ??
      (Number.isFinite(Number(source.x2)) && Number.isFinite(x)
        ? Number(source.x2) - x
        : NaN),
  );
  const height = Number(
    source.height ??
      source.h ??
      (Number.isFinite(Number(source.y2)) && Number.isFinite(y)
        ? Number(source.y2) - y
        : NaN),
  );
  if (![x, y, width, height].every(Number.isFinite)) return null;
  const normalized = Math.max(x, y, width, height) <= 1.5;
...
export default function MeaningPlotPanel({
  ...
  const [meaningNetworkMatcherQueue, setMeaningNetworkMatcherQueue] =
  ...
  const meaningNetworkInterpretiveReadings = useMemo(
    () =>
      (analysisData?.annotationCorrections?.proliferation_decisions || [])
        .map((decision) => decision.governance_status)
        .filter(isMeaningNetworkInterpretiveReadingRecord),
    [analysisData?.annotationCorrections?.proliferation_decisions],
  );
  ...
  const matcherSomMeaningNodes = useMemo(
    () =>
      meaningNetworkNodes
        .filter(isMatcherSomMeaningNode)
        .sort((left, right) => {
          const confidenceDelta = Number(right.maturity?.confidence || 0) - Number(left.maturity?.confidence || 0);
          if (confidenceDelta !== 0) return confidenceDelta;
          return meaningNetworkEvidenceStart(left.evidence_refs) - meaningNetworkEvidenceStart(right.evidence_refs);
        })
        .slice(0, 24),
    [meaningNetworkNodes],
  );
  const matcherSomMeaningEdges = useMemo(
    () =>
      meaningNetworkEdges
        .filter(isMatcherSomMeaningEdge)
  ...
  const runMeaningNetworkMatcher = useCallback(async (
    item: MeaningNetworkNode | MeaningNetworkEdge,
    kind: "node" | "edge",
  ) => {
    if (!selectedVideoId) {
      setMeaningNetworkMatcherQueue({
        anchorKind: kind,
        anchorId: kind === "node" ? (item as MeaningNetworkNode).node_id : (item as MeaningNetworkEdge).edge_id,
        anchorLabel: kind === "node" ? (item as MeaningNetworkNode).label : (item as MeaningNetworkEdge).edge_type,
        requestId: "",
        loading: false,
        error: "Select an analysed video before using Matcher.",
        candidates: [],
      });
      return;
    }
    const node = kind === "node" ? item as MeaningNetworkNode : null;
    const edge = kind === "edge" ? item as MeaningNetworkEdge : null;
    const sourceNode = edge
      ? meaningNetworkNodes.find((candidate) => candidate.node_id === edge.source_node_id)
      : node;
    const targetNode = edge
      ? meaningNetworkNodes.find((candidate) => candidate.node_id === edge.target_node_id)
      : null;
    const anchorId = node?.node_id || edge?.edge_id || `meaning-network:${Date.now()}`;
    const anchorLabel = node
      ? renamedMeaningNetworkMarkers[node.node_id] || node.label
      : `${sourceNode ? renamedMeaningNetworkMarkers[sourceNode.node_id] || sourceNode.label : edge?.source_node_id} -> ${
          targetNode ? renamedMeaningNetworkMarkers[targetNode.node_id] || targetNode.label : edge?.target_node_id
        }`;
    ...
    try {
      const result = await apiService.matchEvidenceProliferation(selectedVideoId, {
        request_id: requestId,
        created_at: new Date().toISOString(),
        video_id: selectedVideoId,
        evidence: {
          overlay_key: anchorId,
          label: anchorLabel,
          source_label: anchorLabel,
          category: node?.node_type || edge?.edge_type || "meaning_network",
          source_panel: "MeaningNetwork",
          source_node_id: node?.node_id,
          source_edge_id: edge?.edge_id,
          source_track_id: attributes.source_track_id || attributes.target_id,
          geometry,
          interval: range ? { start: range.start, end: range.end } : undefined,
          evidence_refs: refs,
        },
        scope: "same_video_open_topology",
        target: node ? meaningNetworkMatcherTargetForNode(node) : "interaction",
        governance: {
      ...
      const message = error instanceof Error ? error.message : "Matcher request failed";
      setMeaningNetworkMatcherQueue({
        anchorKind: kind,
        anchorId,
        anchorLabel,
  ...
  const saveMeaningNetworkMatcherReviewRound = useCallback(async () => {
    if (!selectedVideoId || !meaningNetworkMatcherQueue) return;
    const selectedCandidates = meaningNetworkMatcherQueue.candidates
      .map((candidate) => ({
        candidate,
    ...
    const proliferationDecisions: ProliferationDecision[] = selectedCandidates.map(
      ({ candidate, decision }) => {
        ...
        const candidateLabel = meaningNetworkMatcherCandidateLabel(candidate);
        const rawEvidenceRefs =
          candidate.evidence_refs ||
          candidate.master_object_projection?.evidence_refs ||
          [];
        return {
          decision_id: `meaning-network-matcher:${candidate.candidate_id}`,
          candidate_id: candidate.candidate_id,
          request_id: meaningNetworkMatcherQueue.requestId,
          decision,
          authority_level:
            decision === "confirmed"
              ? "manual_confirmation"
          ...
          source_range_source: hasTime ? "evidence_ref" : undefined,
          candidate_label: candidateLabel,
          applied_label:
            decision === "confirmed" ? meaningNetworkMatcherQueue.anchorLabel : candidateLabel,
          target_evidence_id:
            candidate.evidence_id ||
            String(candidate.master_object_projection?.master_object_id || candidate.candidate_id),
          source_anchors: hasTime
            ? [{
                start_seconds: Number(start.toFixed(3)),
                end_seconds: Number((Number.isFinite(end) ? end : start).toFixed(3)),
                range_source: "evidence_ref",
                geometry: candidate.geometry || undefined,
              }]
            : [],
          evidence_refs: rawEvidenceRefs,
          source_traceback_refs: [
            candidate.master_object_projection?.traceback_ref,
            String(candidate.provenance?.traceback_ref || ""),
          ...
          governance_status: {
            maturity_result:
              decision === "confirmed"
                ...
                : decision === "canceled"
                  ? "analyst_rejected_match"
                  : "analyst_deferred_match",
            matcher_review_round_id: roundId,
            matcher_anchor_id: meaningNetworkMatcherQueue.anchorId,
            matcher_anchor_kind: meaningNetworkMatcherQueue.anchorKind,
          ...
          proliferation_allowed: decision === "confirmed" && hasTime,
          decision_reason: "meaning_network_context_matcher_batch_review",
          created_at: now,
          created_by: "analyst",
        };
      },
    ...
    const reviewedCandidateIds = new Set(
      proliferationDecisions.map((decision) => decision.candidate_id),
    );
    const nextCorrections: AnnotationCorrections = {
      ...existing,
      analysis_id: selectedVideoId,
      ...
      meaning_network_custom_lanes: existing.meaning_network_custom_lanes || [],
      proliferation_decisions: [
        ...currentDecisions.filter((item) => !reviewedCandidateIds.has(item.candidate_id)),
    ...
    reportMeaningNetworkMaturitySave(
      ...
      `${proliferationDecisions.filter((item) => item.decision === "confirmed").length} confirmed, ${
        proliferationDecisions.filter((item) => item.decision === "canceled").length
      } rejected, ${proliferationDecisions.filter((item) => item.decision === "deferred").length} deferred. Confirmed candidates are now user-confirmed truths and are queued for governed projection.`,
    );
    eventBus.emit("meaningNetworkMatcherReviewRoundCommitted", {
      videoId: selectedVideoId,
      request_id: meaningNetworkMatcherQueue.requestId,
      review_round_id: roundId,
      decisions: proliferationDecisions,
      affected_panels: projectionTargets,
    });
    eventBus.emit("analysisCorrectionsChanged", selectedVideoId);
  }, [
  ...
  const persistMeaningNetworkSflDecision = useCallback(async (
    kind: "node" | "edge",
    item: MeaningNetworkNode | MeaningNetworkEdge,
    decision: MeaningNetworkReadingDecision,
    draftOverride?: MeaningNetworkSflDraft,
  ) => {
    const id = kind === "node" ? (item as MeaningNetworkNode).node_id : (item as MeaningNetworkEdge).edge_id;
    const refs = item.evidence_refs || [];
    const range = meaningNetworkEvidenceTimeRange(refs);
    const tracebackRefs = meaningNetworkTracebackRefs(refs);
    const draft = draftOverride || getMeaningNetworkSflDraft(kind, item);
    const now = new Date().toISOString();
    if (!selectedVideoId) {
      reportMeaningNetworkMaturitySave(
    ...
    const readingRecord = buildMeaningNetworkInterpretiveReadingRecord({
      kind,
      id,
      draft,
      decision,
    ...
    const isConfirmedReading = decision === "confirmed";
    const proliferationDecision: ProliferationDecision = {
      decision_id: `meaning-network-reading:${readingRecord.reading_id}`,
      candidate_id: readingRecord.reading_id,
      request_id: `meaning-network-sfl:${selectedVideoId}`,
      decision,
      authority_level: readingRecord.authority_level,
      source_panel: "MeaningNetwork",
      source_verification_status: readingRecord.source_verification_status,
      source_range_source: readingRecord.source_range_source,
      candidate_label: meaningNetworkReadingLensLabel(readingRecord.reading_lens),
      applied_label: readingRecord.plain_language_meaning,
      target_evidence_id: refs[0]?.evidence_id || id,
      source_anchors: readingRecord.source_anchors,
      evidence_refs: refs,
      source_traceback_refs: tracebackRefs,
      projection_targets: projectionTargets,
      proliferates_to: projectionTargets,
      governance_status: {
        ...readingRecord,
        schema: "vaa1.meaning_network_interpretive_reading.v1",
        legacy_confirmation_schema: "vaa1.meaning_network_sfl_confirmation.v1",
        maturity_result: meaningNetworkReadingMaturityResult(decision),
        anchor: "master_schema",
        user_correction_stands: true,
        target_kind: kind,
        target_id: id,
        sfl_layer: readingRecord.sfl_layer,
        sfl_category: readingRecord.sfl_category,
        judgement_group: readingRecord.judgement_group,
        judgement_axis: readingRecord.judgement_axis,
        moral_polarity: readingRecord.moral_polarity,
        virtue_family: readingRecord.virtue_family,
        virtue_axis: readingRecord.virtue_axis,
        virtue_vice_value: readingRecord.virtue_vice_value,
        confirmation_relation: readingRecord.confirmation_relation,
        taxonomy_path: readingRecord.taxonomy_path,
        applied_label: readingRecord.applied_label,
        plain_language_meaning: readingRecord.plain_language_meaning,
        candidate_reading_contributes_to_plot_structure: isConfirmedReading,
        character_meaning_surface_ready: kind === "node",
        source_verification_status: readingRecord.source_verification_status,
        propagation_required: isConfirmedReading,
        partial_propagation_allowed: false,
        corrected_data_proliferates: isConfirmedReading,
        raw_parser_output_retained_as: "candidate_traceback_only",
      },
      proliferation_allowed: isConfirmedReading,
    ...
    setAnalysisData(refreshed);
    reportMeaningNetworkMaturitySave(
      meaningNetworkReadingSaveTitle(decision),
      isConfirmedReading
        ? "Saved reading was sent to the Data Maturity Proliferation regime through the Master Schema authority path."
        : "Saved reading remains a complete governed schema on this node and does not proliferate until confirmed.",
    );
  ...
  const renderMeaningNetworkSflControls = useCallback((
    ...
    const existingReadings = meaningNetworkInterpretiveReadings
      .filter((record) => record.target_kind === kind && record.target_id === id)
      .sort((left, right) => {
    ...
    const virtueAxis = MEANING_NETWORK_SFL_VIRTUES[virtueFamily]?.[draft.virtue_axis]
      ? draft.virtue_axis
      : virtueAxes[0];
    const virtueValues = MEANING_NETWORK_SFL_VIRTUES[virtueFamily]?.[virtueAxis]?.positive || [];
    const ideationalDraft: MeaningNetworkSflDraft = {
      ...draft,
      sfl_layer: "ideational",
      sfl_category: draft.sfl_layer === "ideational" ? draft.sfl_category : MEANING_NETWORK_SFL_IDEATIONAL_CATEGORIES[0],
      confirmation_relation: "confirms",
    };
    const textualDraft: MeaningNetworkSflDraft = {
      ...draft,
      sfl_layer: "textual",
      sfl_category: draft.sfl_layer === "textual" ? draft.sfl_category : MEANING_NETWORK_SFL_TEXTUAL_CATEGORIES[0],
      confirmation_relation: "confirms",
    };
    const interpersonalGroup = draft.sfl_layer === "interpersonal" ? draft.judgement_group : "alignment_positive";
    const interpersonalAxis =
      draft.sfl_layer === "interpersonal"
        ? draft.judgement_axis
        : MEANING_NETWORK_SFL_JUDGEMENT[interpersonalGroup]?.[0] || "acceptance";
    const interpersonalDraft: MeaningNetworkSflDraft = {
      ...draft,
      sfl_layer: "interpersonal",
      sfl_category: interpersonalAxis,
      judgement_group: interpersonalGroup,
      judgement_axis: interpersonalAxis,
      confirmation_relation: draft.sfl_layer === "interpersonal" ? draft.confirmation_relation : "confirms",
    };
    const virtueDraft: MeaningNetworkSflDraft = {
      ...draft,
      sfl_layer: "virtues_and_vices",
      sfl_category: virtueAxis,
      moral_polarity: "virtue",
      virtue_family: virtueFamily,
      virtue_axis: virtueAxis,
      virtue_vice_value: draft.sfl_layer === "virtues_and_vices" && draft.moral_polarity !== "vice"
        ? draft.virtue_vice_value
        : MEANING_NETWORK_SFL_VIRTUES[virtueFamily]?.[virtueAxis]?.positive?.[0] || "kindness",
      confirmation_relation: "morally_legitimizes",
    };
    const viceDraft: MeaningNetworkSflDraft = {
      ...draft,
      sfl_layer: "virtues_and_vices",
      sfl_category: virtueAxis,
      moral_polarity: "vice",
      ...
      virtue_vice_value: draft.sfl_layer === "virtues_and_vices" && draft.moral_polarity === "vice"
        ? draft.virtue_vice_value
        : MEANING_NETWORK_SFL_VIRTUES[virtueFamily]?.[virtueAxis]?.antithesis?.[0] || "cruelty",
      confirmation_relation: "morally_delegitimizes",
    };
    const renderRowActions = (rowDraft: MeaningNetworkSflDraft) => (
      <div className="flex flex-wrap gap-1">
        <button
          type="button"
          onClick={() => void persistMeaningNetworkSflDecision(kind, item, "inspected", rowDraft)}
        ...
        <button
          type="button"
          onClick={() => void persistMeaningNetworkSflDecision(kind, item, "confirmed", rowDraft)}
        ...
        <button
          type="button"
          onClick={() => void persistMeaningNetworkSflDecision(kind, item, "canceled", rowDraft)}
    ...
    return (
      <div className="mt-2 space-y-1" data-vaa1-meaning-network-sfl-sheet="true">
        {existingReadings.length > 0 ? (
          <div className="truncate text-[9px] text-slate-500" data-vaa1-meaning-network-reading-ledger="true">
            Saved readings: {existingReadings.length}
          </div>
        ) : (
          <div className="truncate text-[9px] text-slate-500" data-vaa1-meaning-network-reading-ledger="true">
            Saved readings: 0
        ...
        <div className="grid grid-cols-[96px_minmax(0,1fr)_auto] items-end gap-2" data-vaa1-meaning-network-reading-row="ideational">
          <div className="pb-1 text-[9px] uppercase tracking-[0.12em] text-slate-400">Ideational</div>
          <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
            Subcategory
            <select
              value={ideationalDraft.sfl_category}
              onChange={(event) => updateMeaningNetworkSflDraft(kind, item, {
                ...ideationalDraft,
                sfl_category: event.target.value,
              })}
          ...
          {renderRowActions(ideationalDraft)}
        </div>

        <div className="grid grid-cols-[96px_minmax(0,1fr)_auto] items-end gap-2" data-vaa1-meaning-network-reading-row="interpersonal">
          <div className="pb-1 text-[9px] uppercase tracking-[0.12em] text-slate-400">Interpersonal</div>
          <div className="grid gap-2 md:grid-cols-3">
            <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
              Relation
              <select
                value={interpersonalDraft.confirmation_relation}
                onChange={(event) => updateMeaningNetworkSflDraft(kind, item, {
                  ...interpersonalDraft,
                  confirmation_relation: event.target.value,
                })}
                className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
            ...
            <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
              Judgement group
              <select
                value={interpersonalDraft.judgement_group}
                onChange={(event) => {
                  const nextAxis = MEANING_NETWORK_SFL_JUDGEMENT[event.target.value]?.[0] || "";
                  updateMeaningNetworkSflDraft(kind, item, {
                    ...interpersonalDraft,
                    judgement_group: event.target.value,
                    judgement_axis: nextAxis,
                    sfl_category: nextAxis,
                  });
                }}
                className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
            ...
            <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
              Judgement value
              <select
                value={interpersonalDraft.judgement_axis}
                onChange={(event) => updateMeaningNetworkSflDraft(kind, item, {
                  ...interpersonalDraft,
                  judgement_axis: event.target.value,
                  sfl_category: event.target.value,
                })}
                className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
          ...
          {renderRowActions(interpersonalDraft)}
        </div>

        <div className="grid grid-cols-[96px_minmax(0,1fr)_auto] items-end gap-2" data-vaa1-meaning-network-reading-row="textual">
          <div className="pb-1 text-[9px] uppercase tracking-[0.12em] text-slate-400">Textual</div>
          <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
            Subcategory
            <select
              value={textualDraft.sfl_category}
              onChange={(event) => updateMeaningNetworkSflDraft(kind, item, {
                ...textualDraft,
                sfl_category: event.target.value,
              })}
          ...
          {renderRowActions(textualDraft)}
        </div>

        <div className="grid grid-cols-[96px_minmax(0,1fr)_auto] items-end gap-2" data-vaa1-meaning-network-reading-row="virtues">
          <div className="pb-1 text-[9px] uppercase tracking-[0.12em] text-slate-400">Virtues</div>
          <div className="grid gap-2 md:grid-cols-3">
              <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
                Virtue family
                <select
                  value={virtueDraft.virtue_family}
                  onChange={(event) => {
                    ...
                    updateMeaningNetworkSflDraft(kind, item, {
                      ...
                      virtue_vice_value: MEANING_NETWORK_SFL_VIRTUES[nextFamily]?.[nextAxis]?.positive?.[0] || "",
                    });
                  }}
                  className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    updateMeaningNetworkSflDraft(kind, item, {
                      ...
                      sfl_category: nextAxis,
                      virtue_axis: nextAxis,
                      virtue_vice_value: MEANING_NETWORK_SFL_VIRTUES[virtueFamily]?.[nextAxis]?.positive?.[0] || "",
                    });
                  }}
                  className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
              ...
              <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
                Virtue value
                <select
                  value={virtueDraft.virtue_vice_value}
                  onChange={(event) => updateMeaningNetworkSflDraft(kind, item, {
                    ...virtueDraft,
                    virtue_vice_value: event.target.value,
                  })}
                  className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
          ...
          {renderRowActions(virtueDraft)}
        </div>

        <div className="grid grid-cols-[96px_minmax(0,1fr)_auto] items-end gap-2" data-vaa1-meaning-network-reading-row="vices">
          <div className="pb-1 text-[9px] uppercase tracking-[0.12em] text-slate-400">Vices</div>
          <div className="grid gap-2 md:grid-cols-3">
            <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
              Vice family
              <select
                value={viceDraft.virtue_family}
                onChange={(event) => {
                  const nextFamily = event.target.value;
                  const nextAxis = Object.keys(MEANING_NETWORK_SFL_VIRTUES[nextFamily] || {})[0] || "";
                  updateMeaningNetworkSflDraft(kind, item, {
                    ...
                    virtue_family: nextFamily,
                    virtue_axis: nextAxis,
                    virtue_vice_value: MEANING_NETWORK_SFL_VIRTUES[nextFamily]?.[nextAxis]?.antithesis?.[0] || "",
                  });
                }}
                className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
                data-vaa1-meaning-network-sfl-vice-family-select="true"
              >
                {virtueFamilies.map((family) => (
                  <option key={family} value={family}>{meaningNetworkViceFamilyLabel(family)}</option>
                ))}
              </select>
            </label>
            <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  updateMeaningNetworkSflDraft(kind, item, {
                    ...
                    sfl_category: nextAxis,
                    virtue_axis: nextAxis,
                    virtue_vice_value: MEANING_NETWORK_SFL_VIRTUES[viceDraft.virtue_family]?.[nextAxis]?.antithesis?.[0] || "",
                  });
                }}
                className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
                data-vaa1-meaning-network-sfl-vice-axis-select="true"
              >
                {Object.keys(MEANING_NETWORK_SFL_VIRTUES[viceDraft.virtue_family] || {}).map((axis) => (
                  <option key={axis} value={axis}>{meaningNetworkViceAxisLabel(viceDraft.virtue_family, axis)}</option>
                ))}
              </select>
            </label>
            <label className="text-[9px] uppercase tracking-[0.12em] text-slate-500">
              Vice value
              <select
                value={viceDraft.virtue_vice_value}
                onChange={(event) => updateMeaningNetworkSflDraft(kind, item, {
                  ...viceDraft,
                  virtue_vice_value: event.target.value,
                })}
                className="mt-1 w-full rounded border border-slate-800 bg-[#101010] px-1.5 py-1 text-[10px] normal-case tracking-normal text-slate-100"
                data-vaa1-meaning-network-sfl-vice-value-select="true"
              >
                {(MEANING_NETWORK_SFL_VIRTUES[viceDraft.virtue_family]?.[viceDraft.virtue_axis]?.antithesis || []).map((value) => (
                  <option key={value} value={value}>{value.replaceAll("_", " ")}</option>
                ))}
              </select>
            </label>
          </div>
          {renderRowActions(viceDraft)}
        </div>
      </div>
    );
  }, [
    getMeaningNetworkSflDraft,
    meaningNetworkInterpretiveReadings,
          ...
          <div className="flex min-h-full min-w-[980px] flex-col gap-3">
            ...
            <section
              className={
                dedicatedMeaningNetworkPanel
                  ? "flex min-h-[620px] resize flex-col overflow-visible rounded border border-teal-700/60 bg-[#080b0b] px-4 py-3"
            ...
            >
              ...
              >
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (item) {
                          void runMeaningNetworkMatcher(item, meaningNetworkContextMenu.kind);
                        }
                      }}
                      ...
                      return (
                        <>
                          ...
                          <div className="mt-2 grid gap-1 md:grid-cols-2">
                            <div className="truncate border-b border-slate-900 pb-1 text-[9px] text-slate-500" data-vaa1-meaning-network-compact-source="true">
                              Source: {range ? `${formatTime(range.start)}-${formatTime(range.end)}` : "time pending"} / {meaningNetworkSourceTypes(refs).join(", ") || "no source types"}
                            </div>
                            <div className="truncate border-b border-slate-900 pb-1 text-[9px] text-slate-500" data-vaa1-meaning-network-compact-traceback="true">
                              Traceback: {meaningNetworkTracebackRefs(refs).slice(0, 1).join(", ") || "pending"} / refs {refs.length}
                            </div>
                          </div>
                      ...
                      return (
                        <>
                          ...
                          <div className="mt-2 grid gap-1 md:grid-cols-2">
                            ...
                            <div className="truncate border-b border-slate-900 pb-1 text-[9px] text-slate-500" data-vaa1-meaning-network-compact-traceback="true">
                              Source: {range ? `${formatTime(range.start)}-${formatTime(range.end)}` : "time pending"} / Traceback: {meaningNetworkTracebackRefs(refs).slice(0, 1).join(", ") || "pending"}
                            </div>
                          </div>
                ...
                {(matcherSomMeaningNodes.length > 0 || matcherSomMeaningEdges.length > 0) ? (
                  <div
                    className="mt-2 rounded border border-teal-900/60 bg-teal-950/10 p-2"
                    data-vaa1-meaning-network-matcher-som-strip="true"
                  >
                    <div className="mb-2 flex flex-wrap items-start justify-between gap-2">
                      <div>
                        <div className="text-[9px] uppercase tracking-[0.12em] text-teal-200">
                          Matcher / SOM candidates
                        </div>
                        <div className="mt-0.5 text-[10px] leading-relaxed text-slate-400">
                          Open-topology scanner output is candidate support: jump to source, inspect traceback, then confirm or reject in this graph.
                        </div>
                      </div>
                      <div className="rounded border border-teal-900/70 bg-[#080b0b] px-2 py-1 text-[9px] text-teal-100">
                        {matcherSomMeaningNodes.length} nodes / {matcherSomMeaningEdges.length} edges
                      </div>
                    </div>
                    <div className="grid gap-2 xl:grid-cols-2">
                      <div className="max-h-44 space-y-1 overflow-auto pr-1">
                        {matcherSomMeaningNodes.map((node) => {
                          const label = renamedMeaningNetworkMarkers[node.node_id] || node.label;
                          const confidence = Number(node.maturity?.confidence || 0);
                          return (
                            <div
                              key={`matcher-som-node:${node.node_id}`}
                              className="rounded border border-slate-800 bg-[#101010] px-2 py-1.5"
                              data-vaa1-meaning-network-matcher-som-node="true"
                            >
                              <div className="flex flex-wrap items-start justify-between gap-2">
                                <button
                                  type="button"
                                ...
                                >
                                  ...
                                  <div className="mt-0.5 truncate text-[9px] text-slate-500">
                                    {meaningNetworkNodeKindLabel(node.node_type)} / {meaningNetworkSourceTimeLabel(node.evidence_refs)}
                                  </div>
                                </button>
                                <div className="flex shrink-0 flex-wrap gap-1">
                                  <span className="rounded border border-teal-900/70 bg-[#080b0b] px-1.5 py-0.5 text-[9px] text-teal-100">
                                    {Math.round(confidence * 100)}%
                                  </span>
                                  <button
                                    type="button"
                                    onClick={() => openMeaningNetworkNodeInspector(node)}
                                    className="rounded border border-cyan-800/70 bg-[#080b0b] px-1.5 py-0.5 text-[9px] text-cyan-100 hover:bg-cyan-950/30"
                                  >
                                    Review
                                  </button>
                      ...
                      <div className="max-h-44 space-y-1 overflow-auto pr-1">
                        {matcherSomMeaningEdges.map((edge) => {
                          const sourceNode = meaningNetworkNodes.find((node) => node.node_id === edge.source_node_id);
                          const targetNode = meaningNetworkNodes.find((node) => node.node_id === edge.target_node_id);
                          const sourceLabel = sourceNode ? renamedMeaningNetworkMarkers[sourceNode.node_id] || sourceNode.label : edge.source_node_id;
                          const targetLabel = targetNode ? renamedMeaningNetworkMarkers[targetNode.node_id] || targetNode.label : edge.target_node_id;
                          return (
                            <div
                              key={`matcher-som-edge:${edge.edge_id}`}
                              className="rounded border border-slate-800 bg-[#101010] px-2 py-1.5"
                              data-vaa1-meaning-network-matcher-som-edge="true"
                            >
                              <div className="flex flex-wrap items-start justify-between gap-2">
                                <button
                                  type="button"
                                ...
                                >
                                  ...
                                  <div className="mt-0.5 truncate text-[9px] text-slate-500">
                                    {edge.edge_type.replaceAll("_", " ")} / {meaningNetworkSourceTimeLabel(edge.evidence_refs)}
                                  </div>
                                </button>
                                <div className="flex shrink-0 flex-wrap gap-1">
                                  <span className="rounded border border-teal-900/70 bg-[#080b0b] px-1.5 py-0.5 text-[9px] text-teal-100">
                                    {Math.round(Number(edge.weight || 0) * 100)}%
                                  </span>
                                  <button
                                    type="button"
                                    onClick={() => openMeaningNetworkEdgeInspector(edge)}
                                    className="rounded border border-cyan-800/70 bg-[#080b0b] px-1.5 py-0.5 text-[9px] text-cyan-100 hover:bg-cyan-950/30"
                                  >
                                    Review
                                  </button>
              ...
              {meaningNetworkMatcherQueue ? (
                <section
                  className="mt-2 rounded border border-teal-700/60 bg-[#071011] p-2"
                  data-vaa1-meaning-network-matcher-review-queue="true"
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-[9px] uppercase tracking-[0.12em] text-teal-200">
                      ...
                      <div className="mt-0.5 truncate text-[12px] font-medium text-slate-100">
                        Anchor: {meaningNetworkMatcherQueue.anchorLabel}
                      </div>
                      <div className="mt-0.5 text-[9px] text-slate-500">
                        {meaningNetworkMatcherQueue.anchorKind} / {meaningNetworkMatcherQueue.requestId || "request pending"}
                      </div>
                    </div>
                    <div className="flex shrink-0 items-center gap-1">
                      <div className="flex rounded border border-slate-800 bg-[#080b0b] p-0.5">
                        <button
                          type="button"
                          onClick={() => setMeaningNetworkMatcherView("visual")}
                          className={`rounded px-2 py-0.5 text-[9px] ${
                            meaningNetworkMatcherView === "visual"
                        ...
                        <button
                          type="button"
                          onClick={() => setMeaningNetworkMatcherView("all")}
                          className={`rounded px-2 py-0.5 text-[9px] ${
                            meaningNetworkMatcherView === "all"
                              ? "bg-teal-950/70 text-teal-100"
                              : "text-slate-500 hover:text-slate-200"
                          }`}
                        >
                      ...
                      <span className="rounded border border-teal-900/70 bg-[#080b0b] px-2 py-1 text-[9px] text-teal-100">
                        {meaningNetworkMatcherQueue.loading
                          ? "matching..."
                          : `${meaningNetworkMatcherQueue.candidates.length} candidates`}
                      </span>
                      <button
                        type="button"
                        disabled={
                          meaningNetworkMatcherQueue.loading ||
                          Object.keys(meaningNetworkMatcherSelections).length === 0
                        }
                        onClick={() => void saveMeaningNetworkMatcherReviewRound()}
                        className="rounded border border-emerald-700/70 bg-emerald-950/20 px-2 py-1 text-[9px] text-emerald-100 hover:bg-emerald-950/40 disabled:cursor-not-allowed disabled:opacity-40"
                        data-vaa1-meaning-network-matcher-save-round="true"
                      >
                        Save review round ({Object.keys(meaningNetworkMatcherSelections).length})
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                  ...
                  {meaningNetworkMatcherQueue.error ? (
                    <div className="mt-2 rounded border border-rose-900/70 bg-rose-950/15 px-2 py-2 text-[10px] text-rose-100">
                      {meaningNetworkMatcherQueue.error}
                    </div>
                  ) : null}
                  {meaningNetworkMatcherQueue.loading ? (
                    <div className="mt-2 rounded border border-dashed border-teal-900/70 px-2 py-3 text-[10px] text-slate-400">
                      Scanning source-linked multimodal constellations from this anchor...
                    </div>
                  ) : null}
                  {!meaningNetworkMatcherQueue.loading &&
                  !meaningNetworkMatcherQueue.error &&
                  meaningNetworkMatcherQueue.candidates.length === 0 ? (
                    <div className="mt-2 rounded border border-dashed border-slate-800 px-2 py-3 text-[10px] text-slate-500">
                      No candidates cleared the matcher threshold for this anchor.
                    </div>
                  ) : null}
                  {meaningNetworkMatcherQueue.candidates.length > 0 ? (
                    <div
                      className="mt-2 grid max-h-[640px] grid-cols-1 gap-2 overflow-auto pr-1 xl:grid-cols-2"
                      ...
                      {[
                        {
                          key: "anchor_sample",
                          label: "Known samples",
                          description: "Already-confirmed identity evidence used to guide matching.",
                          roles: ["anchor_sample"] as MeaningNetworkMatcherCandidateRole[],
                          dataAttribute: "known-samples",
                        },
                        {
                          key: "identity_candidate",
                          label: "Confirm these detections",
                          description: "Unknown-person detections that can receive this mature identity.",
                          roles: ["identity_candidate"] as MeaningNetworkMatcherCandidateRole[],
                          dataAttribute: "identity-candidates",
                        },
                        {
                          key: "context_support",
                          label: "Context supporting this match",
                          description: "Transcript, audio, metadata, scene context, and conflicts; inspection only.",
                          roles: ["context_support", "conflict"] as MeaningNetworkMatcherCandidateRole[],
                          dataAttribute: "context-support",
                        },
                      ].map((section) => {
                        const allSectionCandidates = meaningNetworkMatcherQueue.candidates.filter(
                          (candidate) =>
                            section.roles.includes(meaningNetworkMatcherCandidateRole(candidate)),
                        );
                        const visibleSectionCandidates = allSectionCandidates.filter((candidate) => {
                          if (meaningNetworkMatcherView === "all") return true;
                          const start = Number(candidate.time?.start);
                          return (
                            section.key !== "context_support" &&
                            Number.isFinite(start) &&
                            Boolean(meaningNetworkMatcherCandidateBbox(candidate))
                          );
                        });
                        return (
                          <React.Fragment key={section.key}>
                            <div
                              className="col-span-full rounded border border-slate-800 bg-[#080b0b] px-2 py-1.5"
                            ...
                            >
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <span className="rounded border border-slate-800 px-1.5 py-0.5 text-[9px] text-slate-400">
                                  {allSectionCandidates.length}
                                </span>
                              </div>
                              {allSectionCandidates.length > 0 &&
                              visibleSectionCandidates.length === 0 ? (
                                <div className="mt-1 text-[9px] text-slate-600">
                                  Available in All evidence.
                                </div>
                              ) : null}
                            </div>
                            {visibleSectionCandidates.map((candidate) => {
                        ...
                        const end = Number(candidate.time?.end ?? candidate.time?.start);
                        const hasTime = Number.isFinite(start);
                        const bbox = meaningNetworkMatcherCandidateBbox(candidate);
                        const hasGeometry = Boolean(bbox);
                        const stagedDecision =
                          meaningNetworkMatcherSelections[candidate.candidate_id];
                        return (
                          <div
                            key={candidate.candidate_id}
                            className={`rounded border bg-[#101010] px-2 py-2 ${
                              stagedDecision === "confirmed"
                                ? "border-emerald-700/70"
                                : stagedDecision === "canceled"
                                  ? "border-rose-900/70"
                                  : stagedDecision === "deferred"
                                    ? "border-amber-800/70"
                                    : "border-slate-800"
                            }`}
                            data-vaa1-meaning-network-matcher-candidate="true"
                          >
                              ...
                              <button
                                ...
                                className={`group relative mb-2 block aspect-video w-full overflow-hidden rounded border bg-black text-left ${
                                  stagedDecision === "confirmed"
                                    ...
                                    : "border-slate-800 hover:border-teal-500"
                                }`}
                                data-vaa1-meaning-network-matcher-source-preview="true"
                                title={
                                  isDecisionCandidate
                                    ? "Click to check this match. Hover to preview."
                                    : "Known and contextual evidence is inspection-only. Hover to preview."
                                }
                              >
                                <video
                                  src={`${apiService.getDownloadUrl(selectedVideoId, "source_video")}#t=${Math.max(0, start).toFixed(3)}`}
                                  muted
                                  playsInline
                                  preload="metadata"
                                ...
                                {bbox ? (
                                  <span
                                    className="pointer-events-none absolute border-2 border-cyan-300 bg-cyan-300/10 shadow-[0_0_0_1px_rgba(0,0,0,0.7)]"
                                    style={{
                                      left: `${bbox.x * 100}%`,
                                      top: `${bbox.y * 100}%`,
                            ...
                            <div className="flex flex-wrap items-start justify-between gap-2">
                              <button
                                ...
                                disabled={!hasTime}
                                onClick={() => {
                                  if (hasTime && selectedVideoId) openVideoAtTime(selectedVideoId, start);
                                }}
                              ...
                              >
                                ...
                                <div className="mt-0.5 text-[9px] text-slate-500">
                                  {hasTime
                                    ? `${formatTime(start)}-${formatTime(Number.isFinite(end) ? end : start)}`
                                    : "source time pending"}
                                  {" / "}
                                  {hasGeometry ? "BBox/ROI linked" : "geometry pending"}
                                </div>
                              </button>
                              <span className="rounded border border-teal-900/70 bg-[#080b0b] px-2 py-1 text-[9px] text-teal-100">
                                {Math.round(confidence * 100)}%
                            ...
                            <div className="mt-1 flex flex-wrap gap-1">
                              {basis.map((item) => (
                                <span
                                  key={`${candidate.candidate_id}:basis:${item}`}
                                  className="rounded border border-slate-800 bg-[#080b0b] px-1.5 py-0.5 text-[9px] text-slate-400"
                                >
                                  {item}
                                </span>
                              ))}
                            ...
                            <div className="mt-2 flex flex-wrap gap-1">
                              ...
                              <button
                                type="button"
                                disabled={!hasTime || !selectedVideoId}
                                onClick={() => {
                                  if (!hasTime || !selectedVideoId) return;
                                  eventBus.emit("openPanelRequest", {
                              ...
                              <button
                                  ...
                                  openMeaningNetworkTraceback(
                                    {
                                      ...
                                      node_type: "matcher_candidate",
                                      label,
                                      maturity: {
                                        level: "candidate",
                                        authority: "matcher_agent",
                                        confidence,
                                      },
                                      evidence_refs: (candidate.evidence_refs || []) as MeaningNetworkEvidenceRef[],
                                      attributes: {
                                        matcher_anchor_id: meaningNetworkMatcherQueue.anchorId,
                                ...
                                className="rounded border border-amber-800/70 bg-[#080b0b] px-2 py-1 text-[9px] text-amber-100 hover:bg-amber-950/30"
                              >
                                Traceback
                              </button>
                              {isDecisionCandidate ? (
                              ...
                              <button
                                ...
                                className={`rounded border px-2 py-1 text-[9px] disabled:cursor-not-allowed disabled:opacity-40 ${
                                  stagedDecision === "confirmed"
                                    ...
                                    : "border-emerald-800/70 bg-[#080b0b] text-emerald-100 hover:bg-emerald-950/30"
                                }`}
                              >
                                Check match
                              </button>
                              <button
                                ...
                                onClick={() =>
                                  setMeaningNetworkMatcherSelections((current) => ({
                                    ...current,
                                    [candidate.candidate_id]: "canceled",
                                  }))
                                }
                                className={`rounded border px-2 py-1 text-[9px] ${
                                  stagedDecision === "canceled"
                                    ? "border-rose-600 bg-rose-950/50 text-rose-50"
                                    : "border-rose-900/70 bg-[#080b0b] text-rose-100 hover:bg-rose-950/30"
                                }`}
                              >
                                Reject
                              </button>
                              <button
                                ...
                                onClick={() =>
                                  setMeaningNetworkMatcherSelections((current) => ({
                                    ...current,
                                    [candidate.candidate_id]: "deferred",
                                  }))
                                }
                                className={`rounded border px-2 py-1 text-[9px] ${
                                  stagedDecision === "deferred"
                                    ? "border-amber-600 bg-amber-950/40 text-amber-50"
                                    : "border-slate-700 bg-[#080b0b] text-slate-300 hover:border-teal-700 hover:text-teal-100"
                                }`}
                              >
                                Defer
                              </button>
                              <button
                                ...
                                className="rounded border border-slate-800 bg-[#080b0b] px-2 py-1 text-[9px] text-slate-500 hover:text-slate-200"
                              >
                                Clear
                              </button>
                                </>
